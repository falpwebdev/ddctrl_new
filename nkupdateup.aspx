<%@ Page Language="VB" AutoEventWireup="false" CodeFile="nkupdateup.aspx.vb" Inherits="nkupdateup" %>
...
<body>
    <%
        ...
        If Not String.IsNullOrEmpty(Request.QueryString("id")) Then
            ...
            Dim check As String = "SELECT COUNT(ID) FROM T_QUERY WHERE ID = ?"
            connection.Open()
        ...
        If Not String.IsNullOrEmpty(Request.Form("did")) Then
            ...
            Dim q_vnumber() As String = Request.Form.GetValues("q_vnumber")
            Dim q_number() As String = Request.Form.GetValues("q_number")
            Dim q_mdate() As String = Request.Form.GetValues("q_mdate")
            Dim q_pdate() As String = Request.Form.GetValues("q_pdate")
            ...
            Dim ngterm() As String = Request.Form.GetValues("ngterm")
            Dim sfas() As String = Request.Form.GetValues("sfas")
            ...
            Try
                ...
                For i As Integer = 0 To temp
                    ...
                    Dim sql As String = "UPDATE T_QUERY SET Kanban_No=?,Q_VNUMBER=?,Q_NUMBER=?,Q_MDATE=?,Q_PDATE=?,apply=?,check1=?,check2=?,chkreq=?,ngterm=?,sfas=?,ended=?,remark=? WHERE did = ?"
                    Dim cmd As New OleDbCommand(sql, connection)
                    ...
                    If String.IsNullOrEmpty(q_vnumber(i)) Then
                        cmd.Parameters.Add("@Q_VNUMBER", OleDbType.VarChar).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@Q_VNUMBER", OleDbType.VarChar).Value = q_vnumber(i)
                    End If
                    
                    If String.IsNullOrEmpty(q_number(i)) Then
                        cmd.Parameters.Add("@Q_NUMBER", OleDbType.VarChar).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@Q_NUMBER", OleDbType.VarChar).Value = q_number(i)
                    End If
                    
                    If String.IsNullOrEmpty(q_mdate(i)) Then
                        cmd.Parameters.Add("@Q_MDATE", OleDbType.Date).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@Q_MDATE", OleDbType.Date).Value = q_mdate(i)
                    End If
                    
                    If String.IsNullOrEmpty(q_pdate(i)) Then
                        cmd.Parameters.Add("@Q_PDATE", OleDbType.Date).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@Q_PDATE", OleDbType.Date).Value = q_pdate(i)
                    End If
                    ...
                    If String.IsNullOrEmpty(ngterm(i)) Then
                        cmd.Parameters.Add("@ngterm", OleDbType.VarChar).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@ngterm", OleDbType.VarChar).Value = ngterm(i)
                    End If
                    
                    If String.IsNullOrEmpty(sfas(i)) Then
                        cmd.Parameters.Add("@sfas", OleDbType.VarChar).Value = DBNull.Value
                    Else
                        cmd.Parameters.Add("@sfas", OleDbType.VarChar).Value = sfas(i)
                    End If
                    ...
                    If Not String.IsNullOrEmpty(ended(i)) Then
                        sql2 = "UPDATE T_QUERY SET FLAG = 1 WHERE DID = ?"
                    Else
                        sql2 = "UPDATE T_QUERY SET FLAG = 0 WHERE DID = ?"
                    End If
    ...
    %>
        ...
        <div class="row">
            <div class="col-md-12">
                <h3>Unclear Points information update</h3>
            </div>
        </div>
        ...
        <form class="" method="post">
            ...
            <table class="table-nkupdate update2">
                <thead>
                    <tr>
                        <th rowspan="2" nowrap>Base issue no.</th>
                        <th rowspan="2">Kanban No.</th>
                        <th class="col-xs-2">Issue No.</th>
                        <th nowrap>E-mail send date</th>
                        <th>Due Date</th>
                        <th>Answer</th>
                        <th>Check1</th>
                        <th>Check2</th>
                        <th>Chk Req</th>
                        <th>NG terminal</th>
                        <th nowrap>Email Sending Date<br>to FAS</th>
                        <th>Finished Date</th>
                    </tr>
                    <tr>
                ...
                <%
                    If Not String.IsNullOrEmpty(Request.QueryString("id")) Then
                        ...
                        Dim q_vnumber As String = ""
                        Dim q_number As String = ""
                        Dim q_mdate As String = ""
                        Dim q_pdate As String = ""
                        ...
                        Dim ngterm As String = ""
                        Dim sfas As String = ""
                        
                        Dim sql As String = "SELECT did,ID,Kanban_No,Q_VNUMBER,Q_NUMBER,Q_MDATE,Q_PDATE,remark,ENDED,L_UPDATE,FLAG,fdate,apply,check1,check2,chkreq,ngterm,sfas FROM T_QUERY WHERE ID = ? AND FLAG = '0' ORDER BY Kanban_No ASC"
                        Try
                            connection.Close()
                            connection.Open()
                            Dim cmd As New OleDbCommand(sql, connection)
                            ...
                            While reader.Read()
                                ...
                                q_vnumber = reader.Item("Q_VNUMBER").ToString()
                                q_number = reader.Item("Q_NUMBER").ToString()
                                
                                q_mdate = String.Format("{0:yyyy/MM/dd}", reader.Item("Q_MDATE"))
                                q_pdate = String.Format("{0:yyyy/MM/dd}", reader.Item("Q_PDATE"))
                                ...
                                ngterm = reader.Item("ngterm").ToString()
                                sfas = reader.Item("sfas").ToString()
                                %>
                                <tr>
                                    <td class="hide"><input type="text" class="form-control" name="did" value="<% =did %>"></td>
                                    <td><input type="text" class="form-control" name="q_vnumber" value="<% =q_vnumber %>"></td>
                                    <td><input type="text" class="form-control" name="kanban_no" value="<% =kanban_no%>"></td>
                                    <td><input type="text" class="form-control" name="q_number" value="<% =q_number %>"></td>
                                    <td><input type="text" class="form-control" name="q_mdate" value="<% =q_mdate %>" pattern="[0-9]{4}/(0[1-9]|1[012])/(0[1-9]|1[0-9]|2[0-9]|3[01])"></td>
                                    <td><input type="text" class="form-control" name="q_pdate" value="<% =q_pdate%>" pattern="[0-9]{4}/(0[1-9]|1[012])/(0[1-9]|1[0-9]|2[0-9]|3[01])"></td>
                                    ...
                                    <td><input type="text" class="form-control" name="sfas" value="<% =sfas %>" pattern="[0-9]{4}/(0[1-9]|1[012])/(0[1-9]|1[0-9]|2[0-9]|3[01])"></td>
                                    <td><input type="text" class="form-control" name="ended" value="<% =ended %>" pattern="[0-9]{4}/(0[1-9]|1[012])/(0[1-9]|1[0-9]|2[0-9]|3[01])"></td>
                                </tr>
                                <tr>
                                    <td></td>
                                    <td></td>
                                    <td colspan="10"><input type="text" class="form-control" name="remark" value="<% =remark %>"></td>
                                </tr>
                                <%
                            End While
                            reader.Close()
